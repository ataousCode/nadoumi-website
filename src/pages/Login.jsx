...
export default function Login() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (result?.student || result?.token) {
        success('Login successful!')
        const returnTo = location.state?.returnTo || '/student/dashboard'
        navigate(returnTo)
      }
    } catch (err) {
      const errorMessage = studentAuth.error || 'Login failed. Please try again.'
  ...
  const isLoading = loading || studentAuth.loading
  const displayError = error || studentAuth.error
      ...
      <div className="max-w-md mx-auto">
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 md:p-8">
          ...
          <div className="mt-6 text-center text-sm text-gray-600">
            ...
            <Link to="/student/register" className="text-orange-600 hover:text-orange-700 font-medium">
              Sign up
            </Link>
          </div>
        </div>
